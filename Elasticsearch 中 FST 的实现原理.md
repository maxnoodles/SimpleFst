...
1. **确定性**。这意味着在任何给定的状态下，对于任何输入最多有一个路径可以被选择。
2. **非循环**。不能访问一个已经访问过的状态，也就是访问路径不能形成环。
3. **一个转换器**。这意味着有限状态机产生一个与特定输入序列相关联的值。当且仅当输入序列导致状态机处于最终状态时才会获得对应的值。
...
这不是很有趣，但至少值得注意的是 **输出2** 被放置在第一个路径上。从技术上讲，下面的转换器同样是正确的
...
​		但是，如果我们将状态 **0->4** 的输出从 **5** 变为 **3**，那么与键 **thurs** 相关联的值现在就错了。所以我们要把 **5** 和 **3** 的前缀放置位置向后推。在本例中，**5 - 3 = 2**，因此我们在 **状态4** 之后的路径上添加 **2** (**路径u** 还没有真正加入到 **FST** 中)。
...
​		上文描述关联值类型可能有点限制; 如果它们不是**整形(int)**呢? 实际上，可以在 FST 中使用的关联值类型必须含有如下操作的接口:

- 加（Addition）
- 减 (Subtraction)
- 取前缀 (对于整数来说，就是min)

此外，关联值的类型 I 也必须满足一个附加的接口定义，就是保证下列定律成立。
...
​		整形（int）一般满足这个接口定义 (其中前缀定义为 **min** )，额外的好处是它们非常小。当然也可以使用其他类型来满足这个代数定律，只要这个类型能满足以上接口。



##### 6. 实践中的构建

​		**FST** 数据结构的关键应用场所之一是其**存储**和**搜索**大量键的能力。这个目标与上面描述的算法有些不一致，因为它需要将所有冻结状态保存在内存中。即，为了检测 FST 的某些部分是否为通用的后缀，您必须能够搜索实际这个后缀的状态。

​		为此可以使用哈希表，该哈希表提供对任何特定状态的**恒定时间**访问（假设哈希函数良好）。这种方法的问题在于，除了将所有状态实际存储在内存中之外，哈希表通常会产生某种类型的开销。

​		通过牺牲生成的 **FST** 最小化保证，可以减少所需的内存。即，可以维护一个有大小限制的哈希表。这意味着常用的状态保留在哈希表中，而不太常用的状态则被剔除。实际上，在原文作者不科学的实验中，具有约10,000个插槽的哈希表可以达到不错的折中效果，并且非常接近最小值。

​		使用仅存储部分状态的有限哈希表的一个有趣结果是，可以将 **FST** 的构造**流式写入**磁盘上的文件中。也就是说，在构建 **FST** 时，当状态被冻结时，没有理由将它们都保存在内存中。相反，我们可以立即将它们写入磁盘(或套接字，或其他任何东西)。
